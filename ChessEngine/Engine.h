...
class Engine {
    static const int MAXIMUM_NEGAMAX_DEPTH = 14, MAXIMUM_QUIESCENCE_DEPTH = 30, MIN_R = 3, MAX_R = 4, DR = 4;
    chrono::time_point<chrono::steady_clock> start;
    unordered_set<Move> killerMoves[MAXIMUM_NEGAMAX_DEPTH + 1];
    int seconds;
    pair<Move, double> negamax(State& state, int depth);
    double negamax(State& state, int currentDepth, int depth, double alpha, double beta, bool nullMove);
